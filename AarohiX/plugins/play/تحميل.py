...
@app.on_message(command(["يوت", "yt", "تنزيل", "نزل"]))
async def song(_, message: Message):
    try:
        await message.delete()
    except:
        pass
    m = await message.reply_text("↢ يتم البحث الان...انتظر 🕜", quote=True)
    ...
    try:
        if is_valid_youtube_url(query):
            ...
            if not results:
                raise Exception("↢ مفيش حاجه بالاسم دا جرب حاجه تاني .")
    ...
    await m.edit_text("↢ انتظر حتي يتم الانتهاء ")
    audio_file = ''
    try:
    ...
    [
        [InlineKeyboardButton(text="ᴍᴜꜱɪᴄ ᴜᴘᴅᴀᴛᴇᴅ", url=SUPPORT_CHAT)],
    ]
)
